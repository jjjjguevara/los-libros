...
import { LosLibrosClient, type ReadingProgress, type ServerHighlight } from '../server/los-libros-client';
...
export class OPDSSyncService {
  ...
  private clients: Map<string, OPDSClient | LosLibrosClient>;
  ...
  registerServer(config: OPDSServerConfig): void {
    ...
    if (config.type === 'los-libros') {
      this.clients.set(config.id, new LosLibrosClient(this.app, config.catalogUrl));
    } else {
  ...
  async detectCapabilities(catalogUrl: string): Promise<OPDSServerCapabilities> {
    ...
    // Try Los Libros Server health endpoint
    try {
      ...
      if (response.status === 200) {
        ...
        if (data.status === 'ok' || data.server === 'los-libros') {
          capabilities.serverType = 'los-libros';
  ...
  async syncProgress(
    ...
    if (!client || !(client instanceof LosLibrosClient)) {
      return { pushed: false, pulled: false };
    }
  ...
  async testConnection(serverId: string): Promise<boolean> {
    ...
    try {
      if (client instanceof LosLibrosClient) {
