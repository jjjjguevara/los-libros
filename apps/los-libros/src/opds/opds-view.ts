...
export const OPDS_VIEW_TYPE = 'los-libros-opds';
...
export class OPDSView extends ItemView {
  ...
  async onOpen(): Promise<void> {
    ...
    container.empty();
    container.addClass('los-libros-opds-view');
  ...
  async renderBrowser(): Promise<void> {
    ...
    if (!this.catalogUrl) {
      container.createEl('div', {
        cls: 'opds-no-server',
        text: 'Configure a Los Libros server URL in settings to browse OPDS catalogs.',
      });
      return;
    }
