 ...
 *
 * Uses the Los Libros server API for EPUB parsing instead of epub.js.
 */
...
export class LibraryScanner {
  ...
  async scanFolder(folderPath: string): Promise<ScanResult> {
    ...
    if (!serverAvailable) {
      errors.push({
        path: folderPath,
        error: 'Los Libros server is not available. Please start the server to scan books.'
      });
      return { books, errors };
    }
  ...
  async extractMetadata(file: TFile): Promise<Book> {
    if (!this.apiClient) {
      throw new Error('Server not configured. Enable Los Libros server in settings.');
    }
