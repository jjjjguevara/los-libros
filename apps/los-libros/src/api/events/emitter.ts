...
/**
 * Typed event emitter for the Los Libros API
 */
    ...
    if (eventHandlers) {
      eventHandlers.forEach(handler => {
        try {
          ...
          console.error(`[Los Libros] Error in event handler for '${String(event)}':`, error);
        }
      });
    }
    ...
    if (onceEventHandlers) {
      onceEventHandlers.forEach(handler => {
        try {
          ...
          console.error(`[Los Libros] Error in once handler for '${String(event)}':`, error);
        }
      });
