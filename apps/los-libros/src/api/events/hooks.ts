...
/**
 * Hook registry for the Los Libros API
  ...
  async execute<K extends keyof HookContexts>(
    ...
    for (const handler of handlers) {
      try {
        ...
        if (result === false) {
          console.log(`[Los Libros] Hook '${String(hook)}' cancelled operation`);
          return false;
        }
      } catch (error) {
        console.error(`[Los Libros] Error in hook handler for '${String(hook)}':`, error);
