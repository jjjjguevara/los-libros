...
export class ContextMenuRegistry implements ContextMenuAPI {
  ...
  getVisibleItems(context: SelectionContext): ContextMenuItem[] {
    return this.registry.getAll().filter(item => {
      ...
      try {
        ...
        console.error(`[Los Libros] Error evaluating condition for menu item '${item.id}':`, error);
        return false;
      }
    });
  }
  ...
  executeAction(id: string, context: SelectionContext): boolean {
    ...
    if (!item) {
      console.warn(`[Los Libros] Context menu item '${id}' not found`);
      return false;
    }
    ...
    try {
      ...
      console.error(`[Los Libros] Error executing action for menu item '${id}':`, error);
      return false;
    }
  }
