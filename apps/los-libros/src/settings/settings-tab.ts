...
import { LosLibrosClient } from '../server/los-libros-client';
...
export class LibrosSettingTab extends PluginSettingTab {
  ...
  constructor(app: App, plugin: LosLibrosPlugin) {
    super(app, plugin);
    this.plugin = plugin;
  ...
  display(): void {
    ...
    containerEl.createEl('h1', { text: 'Los Libros Settings' });
    ...
    new Setting(containerEl)
      ...
      .setDesc('Connect to a Los Libros server for cloud sync')
    ...
    if (this.plugin.settings.serverEnabled) {
      new Setting(containerEl)
        .setName('Server URL')
        .setDesc('URL of your Los Libros server')
        .addText(text => text
      ...
      new Setting(containerEl)
        ...
        .setDesc('Verify connectivity to the Los Libros server')
        .addButton(button => button
          ...
          .onClick(async () => {
            ...
            try {
              const client = new LosLibrosClient(this.app, url);
              const connected = await client.testConnection();

              if (connected) {
                new Notice('Successfully connected to Los Libros server');
              } else {
    ...
    if (this.plugin.settings.calibreEnabled) {
      ...
      this.syncProgressEl = containerEl.createDiv({ cls: 'los-libros-sync-progress' });
