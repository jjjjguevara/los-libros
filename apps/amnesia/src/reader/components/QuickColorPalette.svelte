<script lang="ts">
  ...
  const colors: { color: HighlightColor; label: string; cssVar: string; fallback: string }[] = [
    { color: 'yellow', label: 'Yellow', cssVar: '--amnesia-highlight-yellow', fallback: 'rgba(254, 243, 199, 0.6)' },
    { color: 'green', label: 'Green', cssVar: '--amnesia-highlight-green', fallback: 'rgba(209, 250, 229, 0.6)' },
    { color: 'blue', label: 'Blue', cssVar: '--amnesia-highlight-blue', fallback: 'rgba(219, 234, 254, 0.6)' },
    { color: 'pink', label: 'Pink', cssVar: '--amnesia-highlight-pink', fallback: 'rgba(252, 231, 243, 0.6)' },
    { color: 'purple', label: 'Purple', cssVar: '--amnesia-highlight-purple', fallback: 'rgba(237, 233, 254, 0.6)' },
  ];
...
{#if visible}
  ...
  >
    {#each colors as { color, label, cssVar, fallback }, index}
      <button
        class="amnesia-quick-palette-color"
