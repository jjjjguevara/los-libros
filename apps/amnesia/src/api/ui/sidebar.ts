...
export class SidebarRegistry implements SidebarAPI {
  ...
  mountView(id: string, container: HTMLElement): boolean {
    ...
    if (!view) {
      console.warn(`[Amnesia] Sidebar view '${id}' not found`);
      return false;
    }
    ...
    try {
      ...
      console.error(`[Amnesia] Error mounting sidebar view '${id}':`, error);
      return false;
    }
  }
  ...
  unmountView(id: string): void {
    ...
    if (cleanup) {
      try {
        ...
        console.error(`[Amnesia] Error unmounting sidebar view '${id}':`, error);
      }
