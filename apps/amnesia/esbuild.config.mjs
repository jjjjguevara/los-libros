...
function bundleServerBinary() {
  ...
  if (!fs.existsSync(serverDir)) {
    console.warn(`⚠️  Server directory not found: ${serverDir}`);
  ...
  try {
    ...
    console.warn("⚠️  Cargo not found. Skipping server build.");
    console.warn("   Install Rust from https://rustup.rs to enable server bundling.");
    return false;
  }

  console.log("\n📦 Building amnesia-server...");
  ...
  try {
    ...
    if (fs.existsSync(binarySource)) {
      ...
      console.log(`✅ Server binary bundled: ${binaryDest} (${sizeMB} MB)`);
      return true;
    } else {
...
if (prod && !skipServer) {
  ...
  if (fs.existsSync(releaseBinary)) {
    ...
    console.log("⚠️  Using debug build of server (run 'cargo build --release' for production)");
  }
  ...
  if (sourceBinary) {
    ...
    console.log(`📦 Server binary copied: ${binaryDest}`);
  }
}
